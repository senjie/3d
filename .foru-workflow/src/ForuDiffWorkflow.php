 ...
 */
final class ForuDiffWorkflow extends ArcanistWorkflow {
...
EOTEXT
  ...
  private function getTasks() {
    ...
    $find_params['order'] = 'order-priority';
    $find_params['status'] = 'status-open';

    return $conduit->callMethodSynchronous('maniphest.query', $find_params);

  }

  private function drawTasks($tasks)
  {

      $table = id(new PhutilConsoleTable())
        ->setShowHeader(false)
        ->addColumn('no', array('title' => '序号'))
...
/* -(  Commit and Update Messages  )----------------------------------------- */
  ...
  private function selectTasks()
  {
    ...
    if ($tasks = $this->getTasks()) {
        ...
        $this->console->writeOut("<bg:green>** %s **</bg> \n\n", "此次diff是否与那个任务有关?");
        $this->drawTasks($tasks);
        if ($selected = $this->console->prompt("任务序号(多个用空格分隔): ")) {
          foreach (explode(" ", $selected) as $select) {
            if (isset($tasks[$select-1])) {
              $ref_tasks .= $tasks[$select-1]['objectName']." ";
            }
          }
        }
    }
    $this->refTasks = trim($ref_tasks);
  }
  ...
  private function getCommitMessageFromUser() {
    ...
    if ($this->refTasks) {
      $template = str_replace("Summary:", "Summary: Ref ".$this->refTasks, $template);
    }

    $template = str_replace("Reviewers:", "Reviewers: #4u2_frontend ", $template);
