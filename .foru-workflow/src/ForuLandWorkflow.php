 ...
 */
final class ForuLandWorkflow extends ArcanistWorkflow {
...
EOTEXT
  ...
  private function updateRelatedTask()
  {
    ...
    $results = $conduit->callMethodSynchronous(
      'edge.search',
      array(
        'sourcePHIDs' => array($rev['phid']),
        'types' => array('revision.task')
      )
    );
    ...
    $transactions = [
        ['type' => 'projects.remove','value' => ['PHID-PROJ-rbyu67thfefmsfzvqv4n']],
        ['type' => 'column', 'value' => ['PHID-PCOL-52xqgqysedj7wr3qnzok']]
    ];

    foreach ($results['data'] as $task) {
        $conduit->callMethodSynchronous('maniphest.edit', [
          'objectIdentifier' => $task['destinationPHID'],
  ...
  /**
   * 确认子模块代码 develop 分支是否为最新
   */
  private function isSubmoduleOutOfDate() {
      $output = shell_exec('git submodule foreach git remote show origin');
      return 0 !== preg_match('/develop pushes to develop \(local out of date\)/', $output);
  }

  public function run() {
    if ($this->isSubmoduleOutOfDate()) {
        throw new  ArcanistUsageException('请将 git 子模块更新到最新版本。');
    }
